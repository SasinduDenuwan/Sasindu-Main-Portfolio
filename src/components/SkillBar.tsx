...
export default function SkillChip({ skill, index }: { skill: Skill; index: number }) {
    ...
    return (
        <motion.div
            ...
            whileTap={{ scale: 0.95 }}
            className="group relative flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl sm:rounded-2xl cursor-default overflow-hidden transition-all duration-300"
            style={{
                background: hovered
                    ? `radial-gradient(circle at center, ${skill.color}18 0%, rgba(255,255,255,0.04) 70%)`
                    : 'rgba(255,255,255,0.04)',
                border: hovered ? `1px solid ${skill.color}50` : '1px solid rgba(255,255,255,0.08)',
                boxShadow: hovered ? `0 0 20px ${skill.color}25, 0 8px 30px rgba(0,0,0,0.3)` : '0 2px 10px rgba(0,0,0,0.2)',
            }}
        >
            <motion.div
                animate={{ opacity: hovered ? 0.2 : 0 }}
                className="absolute inset-0 rounded-xl sm:rounded-2xl blur-lg transition-opacity duration-300"
            ...
            >
                {IconComponent ? (
                    ...
                    <span className="text-sm sm:text-base font-black" style={{ color: skill.color }}>
            ...
            <motion.span
                ...
                className="relative z-10 text-[9px] sm:text-[11px] font-semibold text-center leading-tight"
            >
                {skill.name}
            </motion.span>
